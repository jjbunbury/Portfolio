...
class JHtmlIcon
{
	static function create($category, $params)
	{
		...
		$url = 'index.php?option=com_portfolio&task=article.add&return='.base64_encode(urlencode($uri)).'&a_id=0&catid=' . $category->id;
	...
	static function edit($article, $params, $attribs = array())
	{
		...
		$url	= 'index.php?option=com_portfolio&task=article.edit&a_id='.$article->id.'&return='.base64_encode(urlencode($uri));
